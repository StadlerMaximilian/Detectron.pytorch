...
class RoiDataLoader(data.Dataset):
    def __init__(self, roidb, num_classes, training=True, flags=None):
        self._roidb = roidb
        self._num_classes = num_classes
        self.training = training
        self.DATA_SIZE = len(self._roidb)

        # set flag as attribute on the creation of RoiDataLoader
        self.flags = flags

    def __getitem__(self, index_tuple):
        index, ratio = index_tuple
        single_db = [self._roidb[index]]
        blobs, valid = get_minibatch(single_db, self.flags)
